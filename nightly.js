...
*/
(function(){
	let d = document, AVOTTER_VERSION_STRING = 'v.0.4.10 nightly';
	...
	function addObjectMethodMonitor(obj, name, monitor)
	{
		if (obj && obj[name] && typeof monitor === "function"){
			let original = "avtr_original_" + name, patched = "avtr_patched_" + name,
				monitors = "avtr_"+name+"_monitors", id = Date.now();
			if (! obj[original]){
				obj[original] = obj[name];
				obj[name] = obj[patched] = function(){
			...
			setTimeout(function(){
				m.querySelectorAll('img[src*="/media/"]').forEach(img=>{
					if (img.getElementsByTagName("a").length === 0){
						...
						a.target = "_blank";
						a.style.cssText="position:absolute;background-color:gold;padding:0 0.5em";
						a.textContent = "画像で検索";
	...
	function addUserProfile(user)
	{
		...
		{
			let n = Object.keys(avotter.users).length;
			while (n >= 1000){ n -= 1000; }
			if (n === 0){
				alert("avotter.users.length: " + Object.keys(avotter.users).length);
			}
		}
		/* The following code causes an error at runtime when packed with terser.
		if (Object.keys(avotter.users).length % 1000 === 0){
			alert("avotter.users.length: " + Object.keys(avotter.users).length);
		}
		*/
	}
	...
	function onTransitionComplete()
	{
		...
		if (page.modal){
			...
			if (page.type === "tweet"){
				...
				{
					let e = document.querySelector('div[role="presentation"] > a[href$="/with_replies"]');
					if (e && ! document.querySelector('.avtr-from-to')){
